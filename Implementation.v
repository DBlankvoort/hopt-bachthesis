...
(* Before we can define the context we first need a concept of 'string'. 
   Unfortunately, Coq-HoTT does not natively support strings or even characters.
   To resolve this, we will define strings as list of natural numbers, where we 
   expect some interpreter which maps characters to their natural number 
   ASCII codes and vice versa. *)
Inductive string : Set :=
  | EmptyString : string
  | String : nat -> string -> string.


(* Now we can define our new HIT PT2 *)
Module Export PT2.
Private Inductive PT2 : Type :=
| doc : PT2.
Axiom swapat : string -> string -> nat -> doc = doc.
Axiom indep : forall (s t u v : string) (i j : nat), (i <> j) -> 
                                    ((swapat s t i) @ (swapat u v j) = 
                                     (swapat u v j) @ (swapat s t i)).
Axiom noop : forall (s : string) (i : nat), (swapat s s i) = 1.

Definition PT2_rec {X : Type} (doc' : X) 
                             (swap' : string -> string -> nat -> doc' = doc') 
                             (indep' : forall (s t u v : string) (i j : nat), (i <> j) -> 
                               ((swap' s t i) @ (swap' u v j) = 
                                (swap' u v j) @ (swap' s t i)))
                             (noop' : forall (s : string) (i : nat), (swap' s s i) = 1)
                             (n : PT2) : X :=
match n with
| doc => doc'
end.
Axiom PT2_rec_swapat : forall {X : Type} 
                             (doc' : X) 
                             (swap' : string -> string -> nat -> doc' = doc') 
                             (indep' : forall (s t u v : string) (i j : nat), (i <> j) -> 
                                ((swap' s t i) @ (swap' u v j) = 
                      ...
                      forall (s1 s2 : string) (n : nat), 
                             ap (PT2_rec doc' swap' indep' noop') (swapat s1 s2 n) = (swap' s1 s2 n).
Axiom PT2_rec_noop : forall {X : Type} 
                           (doc' : X) 
                           (swap' : string -> string -> nat -> doc' = doc') 
                           (indep' : forall (s t u v : string) (i j : nat), (i <> j) -> 
                              ((swap' s t i) @ (swap' u v j) = 
                    ...
                    forall (s : string) (n : nat), 
                           transport (fun x => x = 1) (PT2_rec_swapat doc' swap' indep' noop' s s n) 
                                     (ap (ap (PT2_rec doc' swap' indep' noop')) (noop s n))
                           = (noop' s n).
Search "transport" "prod".
Search "ap".
Print ap.
Search (_ -> (_ @ _ = _ @ _)).

Axiom PT2_rec_indep : forall {X : Type} 
                            (doc' : X) 
                            (swap' : string -> string -> nat -> doc' = doc') 
                            (indep' : forall (s t u v : string) (i j : nat), (i <> j) -> 
                               ((swap' s t i) @ (swap' u v j) = 
                                (swap' u v j) @ (swap' s t i)))
                            (noop' : forall (s : string) (i : nat), (swap' s s i) = 1), 
                     forall (s t u v : string) (n m : nat) (neq : n <> m),
                            transport_path_prod' (fun d => (fst d) @ (snd d) = (snd d) @ (fst d)) 
                                                 (PT2_rec_swapat doc' swap' indep' noop' s t n)
                                                 (PT2_rec_swapat doc' swap' indep' noop' u v m)
                                                 (ap (ap (PT2_rec doc' swap' indep' noop')) (indep s t u v n m neq))
                            = (indep' s t u v n m neq).

Definition PT2_ind (P : PT2 -> Type) (b : P doc) (s1 s2 : string) (n : nat) (l : (swapat s1 s2 n) # b = b) (x : PT2) : P x :=
match x with
| doc => b
end.
Axiom PT1_ind_add1 : forall (P : PT2 -> Type) (b : P doc) (s1 s2 : string) (n : nat) (l : (swapat s1 s2 n) # b = b), apD (PT1_ind P b s1 s2 n l) (swapat s1 s2 n) = l.
